 ...
 * 
 * NEW in 009: Defines the contract between autonomy evaluator LLM
 ...
 * 
 * The LLM must return JSON matching this schema. Validation enforces:
 * - Boolean decision (no ambiguity)
 * - Delay bounds (30s - 1 hour)
 * - Brief reason (200 char max for logging)
 * - Optional message content (500 char max)
 */
export const AutonomyEvaluationResponseSchema = z.object({
  shouldSchedule: z.boolean()
    .describe('Whether a follow-up should be scheduled'),
  ...
  followUpType: FollowUpTypeSchema
    .optional()
    .describe('Categorization of why follow-up is needed'),
 ...
 * 
 * Everything the LLM needs to make an informed decision
 */
export const AutonomyEvaluationContextSchema = z.object({
  ...
  timeSinceLastUserMessageMs: z.number()
    .describe('Milliseconds since user last sent a message'),
  
  consecutiveAutonomousMessages: z.number()
    .describe('Count of autonomous messages sent without user response'),
  
  sessionMessageCount: z.number()
    .describe('Total messages in this conversation'),
  
  agentLimits: z.object({
